    ...
    <div className="animate-fade">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', marginBottom: '24px' }}>
        ...
        <div className="glass-panel" style={{ padding: '32px', height: '400px' }}>
          <h3 style={{ fontSize: '18px', marginBottom: '32px' }}>Évolution de la Communauté</h3>
        ...
        <div className="glass-panel" style={{ padding: '32px', height: '400px' }}>
          <h3 style={{ fontSize: '18px', marginBottom: '32px' }}>Top 5 Tontines (Volume FCFA)</h3>
          <ResponsiveContainer width="100%" height="80%">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
        ...
        <div className="glass-panel" style={{ padding: '32px', height: '400px', display: 'flex', flexDirection: 'column' }}>
          <h3 style={{ fontSize: '18px', marginBottom: '32px' }}>Santé des Paiements</h3>
        ...
        <div className="glass-panel" style={{ padding: '32px', background: 'linear-gradient(135deg, rgba(155, 81, 224, 0.05), transparent)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
             <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: 'rgba(46, 204, 113, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#2ecc71' }}>
                  <TrendingUp size={20} />
                </div>
                <div>
                  <p style={{ margin: 0, fontSize: '14px', color: 'var(--text-muted)' }}>Engagement</p>
                  <p style={{ margin: 0, fontWeight: '600' }}>Taux moyen de {Math.round((data.paiements.filter(p=>p.statut==='confirme').length / (data.paiements.length || 1)) * 100)}% de réussite des paiements.</p>
                </div>
             </div>
             
             <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: 'rgba(155, 81, 224, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--primary)' }}>
                  <Users size={20} />
                </div>
                <div>
                  <p style={{ margin: 0, fontSize: '14px', color: 'var(--text-muted)' }}>Densité</p>
                  <p style={{ margin: 0, fontWeight: '600' }}>Moyenne de {Math.round(data.users.length / (data.tontines.length || 1))} membres par tontine.</p>
                </div>
             </div>

             <div style={{ marginTop: '20px', padding: '20px', background: 'rgba(0,0,0,0.2)', borderRadius: '12px', borderLeft: '4px solid var(--primary)' }}>
                <p style={{ margin: 0, fontSize: '13px', fontStyle: 'italic', color: 'var(--text-muted)' }}>
                  "L'analyse montre une corrélation positive entre le nombre de membres et la vélocité des transactions. La plateforme maintient un ratio de solvabilité sain."
                </p>
             </div>
          </div>
        </div>
